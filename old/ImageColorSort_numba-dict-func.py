...
def color_comp():
	...
	try:
		...
		for pixel in pixel_data:
			...
			if converted in color_list:
				weight_list[color_list.index(converted)] += 1
			else:
				color_list += [converted]
				weight_list += [1]
		return color_list[100], weight_list[100]
		#cw_array = np.insert((np.array(color_list)), 3, weight_list, axis=1)
		#return cw_array[100]
	...
	def color_in_img(img):
		...
		def color_count(cw):
			...
			colorname = array(["red", "orange", "yellow", "green", "cyan", \
								"blue", "purple", "pink"])
			indices = array([8])
			for pixel in cw:
				...
				for elem in indices[1:]:
					counter[elem] += 1*pixel[3]
			return counter
